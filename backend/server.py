...
def ratio_size(r,q):
    return {('16:9','480p'):(832,480),('16:9','720p'):(1280,720),('16:9','1080p'):(1920,1080),('9:16','480p'):(480,832),('9:16','720p'):(720,1280),('9:16','1080p'):(1080,1920),('1:1','480p'):(624,624),('1:1','720p'):(960,960),('1:1','1080p'):(1440,1440),('4:5','480p'):(576,720),('4:5','1080p'):(864,1080)}.get((r,q),(1280,720))

class GenerateRequest(BaseModel):
    prompt:str=Field(min_length=1,max_length=12000); negative_prompt:str=''; duration:int=Field(5,ge=1,le=60); fps:int=Field(24,ge=8,le=60); aspect_ratio:str='16:9'; quality:str='720p'; camera:str='Auto'; model:str='Local ComfyUI'; scenes:list=[]

def load_workflow():
    p=os.getenv('COMFYUI_WORKFLOW_JSON','').strip()
    if not p: raise RuntimeError('COMFYUI_WORKFLOW_JSON is not configured. Export an API-format Wan workflow from ComfyUI.')
    path=Path(p)
...
def patch_workflow(wf,req):
    ...
    negative=req.negative_prompt or 'blurry, distorted, flicker, duplicate subjects, bad anatomy'; width,height=ratio_size(req.aspect_ratio,req.quality); frames=max(9,min(241,round(req.duration*req.fps/4)*4+1))
    for node in wf.values():
        if not isinstance(node,dict):continue
        inp=node.get('inputs',{}); cls=str(node.get('class_type','')).lower()
        if 'cliptextencode' in cls and isinstance(inp.get('text'),str):inp['text']=negative if 'negative' in cls else prompt
        for k in list(inp):
            lk=str(k).lower()
...
def history(pid):
    try:
        r=requests.get(comfy_url()+'/history/'+pid,timeout=15);return r.json().get(pid) if r.ok else None
    ...
    for node in h.get('outputs',{}).values():
        for key in ('gifs','videos'):
            for item in node.get(key,[]) if isinstance(node.get(key,[]),list) else []:
                if item.get('filename'):return '/api/comfy-file?filename='+quote(item['filename'])+'&subfolder='+quote(item.get('subfolder',''))+'&type='+quote(item.get('type','output'))
    return None
...
def scan():
    ...
    try:
        r=requests.get(comfy_url()+'/models',timeout=8);models=r.json() if r.ok else {};names=[]
        ...
        s=' '.join(map(str,names)).lower();d=out['models']['detected'];d['wan5b']='wan2.2_ti2v_5b' in s;d['wan14b']='wan2.2' in s and '14b' in s;d['umt5']='umt5' in s;d['wan_vae']='wan' in s and 'vae' in s
...
def requirements():return {'official_template':'Wan2.2 5B video generation','template_location':'ComfyUI → Workflow → Browse Templates → Video','models':['wan2.2_ti2v_5B_fp16.safetensors','wan2.2_vae.safetensors','umt5_xxl_fp8_e4m3fn_scaled.safetensors']}

@app.post('/api/setup/test')
def setup_test():
    if not comfy_reachable():raise HTTPException(503,'ComfyUI is not reachable.')
    if not os.getenv('COMFYUI_WORKFLOW_JSON','').strip():return {'ok':True,'level':'connection','message':'ComfyUI is reachable. Configure an API-format workflow to run generation.'}
    try:
        pid=queue_comfy(patch_workflow(load_workflow(),GenerateRequest(prompt='Realizer connection test',duration=1,fps=8,quality='480p')));JOBS[pid]={'status':'queued','progress':0,'test':True};return {'ok':True,'level':'generation','job_id':pid}
    except Exception as e:raise HTTPException(502,str(e))

@app.post('/api/generate')
def generate(req:GenerateRequest):
    if req.model=='Hugging Face Provider':raise HTTPException(501,'Hosted provider is not configured.')
